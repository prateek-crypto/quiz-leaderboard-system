...
Calls the quiz API 10 times (once every 5 seconds) with poll values 0–9
Collects all the score events from each response
Skips duplicate events — same round + same participant = already counted
Adds up the real scores per participant
Sorts participants by total score (highest first)
Submits the leaderboard once to the submit endpoint
